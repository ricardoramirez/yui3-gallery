 ...
 */
YUI.add('notifications', function(Y) {
	...
	Y.Notifications.prototype.render = function() {
		...
		Y.DOM.setStyle(pop, 'display', 'none');
	...
	Y.Notifications.prototype.switchStatus = function(tab, pop) {
		...
		if (popNode.getStyle('display') === 'none')	 // opening
		{
			popNode.setStyle('display', '');
			tabNode.addClass('open');
			tabNode.removeClass('closed');
		}
		else
		{
			popNode.setStyle('display', 'none');
...
}, 'gallery-2010.03.20', { requires: [ 'dom', 'node', 'json-parse', 'io-base' ] } );
